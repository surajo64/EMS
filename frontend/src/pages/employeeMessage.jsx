...
const EmployeeMessage = () => {
    const { token, backendUrl, user, fetchEmployeeMessage, emplMessages, setEmplMessages, } =
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setEmplMessages((prev) => [...prev, ...data.messages]);
                setMessage("");
                setTitle("");
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (data.success) {
                ...
                setEmplMessages((prev) => prev.filter((m) => m._id !== id));
    ...
    // ✅ Search filter
    useEffect(() => {
        const filtered = (emplMessages || []).filter((m) =>
            m.title.toLowerCase().includes(searchTerm.toLowerCase())
        );
        setFilteredMessages(filtered);
        setCurrentPage(1);
    }, [searchTerm, emplMessages]);
    ...
    useEffect(() => {
        setFilteredMessages(emplMessages || []);
        setCurrentPage(1);
    }, [emplMessages]);
